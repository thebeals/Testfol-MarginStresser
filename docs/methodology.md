...
`QQUPSIM` is the realized-tracker counterpart to the theoretical
`NDXMEGASIM?L=2&E=0.95` expression. Its pre-inception history uses a separately
reconstructed NDXMEGA **price-return** series, then applies the Testfol-compatible
2x LETF formula with `SW=1.10`, `SP=0.40`, daily `DFF`, and `E=0.95`. From the
first official common anchor it uses the official `NASDAQNDXMEGA` price-index
returns; after QQUP begins trading it preserves actual adjusted QQUP returns.
The expense ratio is therefore applied once to synthetic history and is not
deducted again from actual QQUP prices.
...
-   **Return conventions**: `NDXMEGASIM` is dividend-adjusted (total return). `NDXMEGAPRICESIM` uses raw closes (price return) and exists specifically as the pre-launch underlying for `QQUPSIM`.
